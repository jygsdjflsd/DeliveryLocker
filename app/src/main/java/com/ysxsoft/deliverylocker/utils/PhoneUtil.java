...
public class PhoneUtil {
    ...
    public static int getSimCount() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP_MR1) {
            try {
                SubscriptionManager mSubscriptionManager = (SubscriptionManager) MyApplication.getApplication().getSystemService(Context.TELEPHONY_SUBSCRIPTION_SERVICE);
...
//        List<SimInfo> simInfos = ConvertUtils.removeDuplicateWithOrder(infos);//去重
...
//        }
    ...
    private static String getReflexMethod(String predictedMethodName) throws MethodNotFoundException {
        String result = null;
        TelephonyManager telephony = (TelephonyManager) MyApplication.getApplication().getSystemService(Context.TELEPHONY_SERVICE);
        try {
            ...
            LogUtil.d(e.fillInStackTrace().getMessage());
    ...
    public static String getAndroidId () {
        String ANDROID_ID = Settings.System.getString(MyApplication.getApplication().getContentResolver(), Settings.System.ANDROID_ID);
        return ANDROID_ID;
    }
}
